<?php
    ...
    if($method=="PUT"){
        ...
        try {
            ...
            if (!empty($senha)){
                $sql = "UPDATE usuarios SET e_mail=:e_mail, senha=:senha, habilita=:habilita 
                        WHERE pk_id=:id";
                $stmt = $conn->prepare($sql);
                $stmt->bindParam(":e_mail", $e_mail);
                $stmt->bindParam(":senha", $senha);
                ...
                $sql = "UPDATE usuarios SET e_mail=:e_mail, habilita=:habilita 
                        WHERE pk_id=:id";
                $stmt = $conn->prepare($sql);
                $stmt->bindParam(":e_mail", $e_mail);
                $stmt->bindParam(":habilita", $habilita);
